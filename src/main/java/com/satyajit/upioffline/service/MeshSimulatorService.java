...
public class MeshSimulatorService {
    ...
    // drops a packet onto the sender's device — entry point into the mesh
    ...
    // one round of gossip — every device shares all its packets with every other device
    // snapshot taken at round start so packets don't travel through all devices in one step
        ...
        for (VirtualDevice src : deviceList) {
            for (MeshPacket pkt : snapshot.get(src.getDeviceId())) {
                ...
                for (VirtualDevice dst : deviceList) {
                    ...
                    // create a copy with decremented TTL for the receiving device
    ...
    // current packet count per device — used by dashboard
    ...
    // collects packets from bridge nodes — these get uploaded to the backend
